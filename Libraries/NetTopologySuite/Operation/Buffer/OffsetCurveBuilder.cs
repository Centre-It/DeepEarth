...
namespace GisSharpBlog.NetTopologySuite.Operation.Buffer
{
    ...
    {        
        ...
        */
        ...
        private List<ICoordinate> ptList;
        ...
        {
            IList lineList = new List<object>();
        ...
        private ICoordinate[] Coordinates
        {
            get
            {
                ...
                if (ptList.Count > 1)
                {
                    ICoordinate start = ptList[0];
                    ICoordinate end = ptList[1];
                    if (!start.Equals(end)) 
                        AddPt(start);
                }
                ICoordinate[] coord = ptList.ToArray();
                return coord;
            }
        }
